...
###Test if data_import makes data.frame objects
```{r DataImport_Test, echo=TRUE}
NameOfCSV <- c("dummy.csv.file1","dummy.csv.file2")
test <- data_import("NameOfCSV")
...
###Test that data_import makes dataframes of correct dimensions

```{r DataImport_Test_2, echo=TRUE}
ExpectRows <- 42
ExpectCols <- 16
test <- data_import("NameOfCSV[1]")
...
```{r data_fill_1, echo=TRUE}
ExpectDatRows <- 200
datasets <- #names of dummy datasets from import function tested above
soil_test <- Data_fill(datasets)
if(nrow(soil_test) == ExpectDatRows)
{
  print("passed")
}else
{
